...
export const getRandomMessageText = () =>
  template(chance.pick(templates))({
...
export const initializeDB = () => {
  ...
  users.forEach(user => {
    for (let i = 0; i < contactCount; i++) {
      user.contacts.push(
        chance.pick(
          users.filter(
            ({ id }) => id !== user.id && !user.contacts.includes(id)
          )
        ).id
      );
    }
  ...
  users.forEach(user => {
    user.activeChannel = chance.pick(
      channels.filter(channel => channel.participants.includes(user.id))
    ).id;
  });
};
